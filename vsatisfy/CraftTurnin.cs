...
public sealed class CraftTurnin
{
    ...
    private static unsafe uint GetCrafter()
    {
        return Plugin.Config.CraftJobType switch
        {
            ...
            Config.JobChoice.当前职业 => Service.LuminaRow<ClassJob>(Service.ClientState.LocalPlayer?.ClassJob.RowId ?? 0)?.RowId ?? Plugin.Config.SelectedCraftJob,
            Config.JobChoice.等级最低的职业 => Service.LuminaSheet<ClassJob>()?.Where(c => c.ClassJobCategory.RowId == 33).OrderBy(c => PlayerState.Instance()->ClassJobLevels[c.ExpArrayIndex]).First().RowId ?? Plugin.Config.SelectedCraftJob,
            Config.JobChoice.等级最高的职业 => Service.LuminaSheet<ClassJob>()?.Where(c => c.ClassJobCategory.RowId == 33).OrderByDescending(c => PlayerState.Instance()->ClassJobLevels[c.ExpArrayIndex]).First().RowId ?? Plugin.Config.SelectedCraftJob,
